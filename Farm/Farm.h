...
public:
  ...
  // Preconditions:  None
  // Postconditions: Changes season
  // Note: In order to remove an AgItem from the m_farm vector,
  //       you are allowed to use an iterator (which allows you to use the
  //       "erase" function.)
  // for (vector<AgItem*>::iterator it = m_farm.begin() ;
  //     it != m_farm.end(); ++it)
  void Tick(int);
